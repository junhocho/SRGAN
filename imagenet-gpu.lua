...
local filtsizes = {11,5,3,3,3,1,1,1}
local paddings = {4,4,2,2,2}
local strides = {4,1,1,1,1,1,1,1,1}
local poolsizes = {1,2,2,1,2}
...
for i = 1,ncolumns do 
   ...
   -- stage 1: conv+max
   submodel:add(nn.SpatialConvolutionCUDA(nfeatures[0], nfeatures[1], filtsizes[1], filtsizes[1], strides[1], strides[1], paddings[1]))
   submodel:add(nn.Threshold(0,0))
   submodel:add(nn.SpatialMaxPoolingCUDA(kersizes[1],kersizes[1],poolsizes[1],poolsizes[1]))
   
   -- stage 2: conv+max
   submodel:add(nn.SpatialConvolutionCUDA(nfeatures[1], nfeatures[2], filtsizes[2], filtsizes[2], strides[2], strides[2], paddings[2]))
   submodel:add(nn.Threshold(0,0))
   submodel:add(nn.SpatialMaxPoolingCUDA(kersizes[2],kersizes[2],poolsizes[2],poolsizes[2]))
   
   -- stage 3: conv
   submodel:add(nn.SpatialConvolutionCUDA(nfeatures[2], nfeatures[3], filtsizes[3], filtsizes[3], strides[3], strides[3], paddings[3]))
   submodel:add(nn.Threshold(0,0))
   submodel:add(nn.SpatialMaxPoolingCUDA(kersizes[3],kersizes[3],poolsizes[3],poolsizes[3]))
   
   -- stage 4: conv
   submodel:add(nn.SpatialConvolutionCUDA(nfeatures[3], nfeatures[4], filtsizes[4], filtsizes[4], strides[4], strides[4], paddings[4]))
   submodel:add(nn.Threshold(0,0))
   submodel:add(nn.SpatialMaxPoolingCUDA(kersizes[4],kersizes[4],poolsizes[4],poolsizes[4]))
   
   -- stage 5: conv
   submodel:add(nn.SpatialConvolutionCUDA(nfeatures[4], nfeatures[5], filtsizes[5], filtsizes[5], strides[5], strides[5], paddings[5]))
   submodel:add(nn.Threshold(0,0))
   submodel:add(nn.SpatialMaxPoolingCUDA(kersizes[5],kersizes[5],poolsizes[5],poolsizes[5]))
...
batch_size = 128 -- for CUDA tests
testbatch = torch.Tensor(batch_size,3,224,224):cuda()
